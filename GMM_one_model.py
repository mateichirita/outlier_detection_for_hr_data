...
def main():
    ...
    for x in combinations:
        ...
        for i in range(2, 13):
            ...
            pipe = Pipeline([("trans", Transformer(mfcc_components = 5, pca_components = 0, transformations = [np.mean, np.median])), 
                             ("gmm", GaussianMixture(n_components = i * 4))])

            pipe.fit(train_matrix)
            
            score = custom_scorer(pipe, test_matrix)
            scores.append(score)
        ...
        with open("results/GMM_new/result_NvsN_mfcc_one_3h.json", "w") as outfile:
