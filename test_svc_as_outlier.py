...
def main():
    ...
    combinations: list = list(itertools.combinations(allSubjects, 6))
    np.random.seed(42)
    ...
    for x in combinations:
        ...
        train, test = TrainTestSplitterRawData(1, 0.8, auth = list(x[ 1]), nonAuth = list(x[1: ])).splitBeforeWindowing2(subjects)
        ...
        pipe = Pipeline([("pca", PCA()), ("svc", SVC(gamma="auto"))])
        param_grid = {
            "pca__n_components": [4, 10, 20, 25, 42, 100, 150, 200, 250, 300, 350, 400, 450, 500],
            # "svc__gamma": ["scale", "auto"],
            "svc__C": [0.1, 0.5, 0.6, 0.8, 1, 2, 5, 10],
        }
        ...
        print(result)
        result.to_csv("result_SVM_C.csv", index=False)
