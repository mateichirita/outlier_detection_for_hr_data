...
subjectsPath = join(pathlib.Path().resolve(), "data\\me_time_fused\\windowed_subjects\\1_hour_no_stride")
...
def normalizer_transform(data: DataFrame, scaler: StandardScaler) -> dict[str, DataFrame]:
    ...
    data.iloc[:, data.columns != "label"] = matrix_norm

    return data

def custom_scorer_acc(estimator, X, y):

    prob = estimator.predict(test_matrix_global)

    return accuracy_score(test_global["label"], prob)

def custom_scorer_auc(estimator, X, y):
...
def main():
    ...
    for x in combinations:
        ...
        if len(train_matrix) < 1500 or len(test_matrix) < 500:
            continue


        pipe = Pipeline([("trans", Transformer()), ("gmm", OneClassSVM())])
        param_grid = {
            ...
            "gmm__nu": [0.1, 0.3, 0.5, 0.8]
        }

        grid = GridSearchCV(pipe, param_grid, cv=[(slice(None), slice(None))], scoring=custom_scorer_auc, n_jobs=-1, refit = True)
        ...
        taken += 1
        result.to_csv("results/SVM_new/result_1vsN_mfcc_auc.csv", index=False)
